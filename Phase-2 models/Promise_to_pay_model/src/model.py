...
class PromiseToPayModel(nn.Module):
    def __init__(self, vocab_size=10000, d_model=256, nhead=8, num_layers=4, dropout=0.3):
        ...
        # Translation branch: Embedding + Transformer
        ...
        # Transcription branch: Embedding + Transformer
        ...
        self.classifier = nn.Linear(d_model, 4)  # 4 Promise to Pay categories
        self.keyword_classifier = nn.Linear(d_model, 3)  # B-KEY, I-KEY, O
    ...
    def forward(self, audio, audio_lengths, trans_tokens, transcr_tokens, trans_len):
        ...
        audio = self.audio_flatten(audio)  # [batch, time, d_model]
        ...
        audio_repr = audio.mean(dim=1)  # [batch, d_model]
        ...
        trans_repr = trans.mean(dim=1)  # [batch, d_model]
        ...
        transcr_repr = transcr.mean(dim=1)  # [batch, d_model]
        ...
        # Classification
        class_logits = self.classifier(fused)  # [batch, 4]

        # Keyword extraction (sequence labeling on Translation)
        keyword_logits = self.keyword_classifier(trans)  # [batch, seq_len, 3]

        return class_logits, keyword_logits