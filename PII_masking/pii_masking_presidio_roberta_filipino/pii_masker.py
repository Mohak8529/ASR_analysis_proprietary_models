...
class FilipinoPIIMasker:
    def __init__(self, surname_file: str = "filipino_surnames.txt"):
        ...
        # Configure Presidio NLP engine for Tagalog using Transformers
        # Note: Ensure transformers==4.21.0 and torch==1.12.0 are installed in your environment
        configuration = {
            "nlp_engine_name": "transformers",
            "models": [{"lang_code": "tl", "model_name": "Davlan/xlm-roberta-base-ner-hrl"}]
        }
        try:
            provider = NlpEngineProvider(nlp_configuration=configuration)
            nlp_engine = provider.create_engine()
            print(f"Using NLP engine: {type(nlp_engine).__name__} with model: {configuration['models'][0]['model_name']}")
        except Exception as e:
            print(f"Error initializing TransformersNlpEngine: {e}. Falling back to default AnalyzerEngine.")
            nlp_engine = None

        # Initialize Presidio analyzer with the configured NLP engine or fallback
