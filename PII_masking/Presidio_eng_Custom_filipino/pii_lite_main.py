...
def clean_incomplete_tags(transcription: List[Dict]) -> List[Dict]:
    """
    Clean incomplete/malformed PERSON tag fragments without touching valid [PERSON], [ADDRESS], [FIGURE] tags.
    """
    cleaned_transcription = []

    valid_tags = ['[PERSON]', '[ADDRESS]', '[FIGURE]']
    tag_placeholders = {tag: f'__{i}__' for i, tag in enumerate(valid_tags)}

    # Fragments to remove
    fragments = ['RSON', 'ERSON', 'SON', 'ON', 'N']
    fragments_pattern = '|'.join(fragments)

    # Patterns
    address_cleanup_pattern = re.compile(
        rf'(\[ADDRESS])(?:{fragments_pattern})]', flags=re.IGNORECASE
    )
    standalone_fragment_pattern = re.compile(
        rf'(?:{fragments_pattern})]', flags=re.IGNORECASE
    )
    ...
    for entry in transcription:
        ...
        # Clean malformed fragments after [ADDRESS]
        text = address_cleanup_pattern.sub(r'\1', text)

        # Clean any remaining malformed fragments
        text = standalone_fragment_pattern.sub('', text)

        # Restore protected tags
        for tag, placeholder in tag_placeholders.items():
            text = text.replace(placeholder, tag)

        cleaned_entry["dialogue"] = text
...
def main():
    ...
    transcription_path = "test_main/taglish_calls/mahe_test_3/Transcription 1"
    audio_path = "test_main/taglish_calls/mahe_test_3/06a2c371-a648-4abf-b618-92c2705488aa_predictive-09351646305-882000067-20250211-121146-1739247082.3125079_613821978277.wav"
    beep_path = "beep.wav"
    ...
    try:
        ...
        print("THIS IS THE MASKED TEXT that needs to cleaned:", masker.masked_transcription)
        ...
        masked_transcription = clean_incomplete_tags(masker.masked_transcription)
