...
# Fuzzy logic evaluation function (restored to original binary output)
...
def calculate_days_overdue(call_date_str: str, due_date_str: str) -> int:
    ...
    if len(parts) == 2 and parts[0].capitalize() in ["January", "February", "March", "April", "May", "June",
                                                    "July", "August", "September", "October", "November", "December"]:
        due_date_str = f"{parts[1]} {parts[0]}"  # Reorder to "5 April"
    call_date = datetime.strptime(call_date_str, "%d/%m/%Y")
    due_date = datetime.strptime(due_date_str, "%d %B")
...
def evaluate_criteria(transcription: List[Dict[str, Any]]) -> Dict[str, Any]:
    output = {
        ...
        "total_points": "0",
        "type_of_postdue_collection": None
    }
    ...
    if due_date_str:
        ...
        if days_overdue < 30:
            output["type_of_postdue_collection"] = "Postdue Collections Less Than 30 days"
        elif 30 <= days_overdue < 60:
            output["type_of_postdue_collection"] = "Postdue Collections Greater Than 30 days"
        else:
            output["type_of_postdue_collection"] = "Late Collections Greater Than 60 days"
    else:
        output["type_of_postdue_collection"] = "Postdue Collections Less Than 30 days"
    ...
    # Second pass: Evaluate criteria (restored original keywords)
    for entry in transcription:
        dialogue = entry["dialogue"]
        speaker = entry["speaker"]
        start_time = entry["startTime"]
